...
type OrderData = {
  ...
  frete: number; total: number; createdAt: string; supplierId: string; items: OrderItem[];
};
...
type Props = { initialOrders: OrderData[]; customerId: string; cpf: string; customerNome: string };

export default function ClienteOrdersClient({ initialOrders, cpf, customerNome }: Props) {
        ...
        <div className="flex flex-col gap-4">
          ...
          {orders.length === 0 ? (
            <div className="bg-white dark:bg-zinc-900 rounded-xl border border-zinc-200 dark:border-zinc-800 p-8 text-center">
              <p className="text-zinc-500 mb-4">Nenhum pedido encontrado.</p>
              <Link href="/orders/new" className="h-10 px-6 rounded-full bg-orange-500 text-white text-sm font-semibold hover:bg-orange-600 transition-colors inline-flex items-center">
            ...
            orders.map((order, index) => (
              <div key={order.id} className="bg-white dark:bg-zinc-900 rounded-xl border border-zinc-200 dark:border-zinc-800 p-6">
                ...
                <div className="flex flex-wrap gap-2 pt-3 border-t border-zinc-100 dark:border-zinc-800">
                  ...
                  {!["PENDING", "PAID", "CANCELLED", "DELIVERED"].includes(order.status) && (
